...
async function runCrataeguttTest(page) {
    let retryCount = 0;
    const maxRetries = 3;

    while (retryCount < maxRetries) {
        try {
            console.log("Starting Crataegutt test...");
            await page.goto("https://www.schwabe-fachkreise.de/fortbildungen/online-trainings/produkt/crataegutt-450-mg-herz-kreislauf-tabletten/training/crataegutt-450-mg-herz-kreislauf-tabletten-training/wissenstest/1/", {
            ...
            console.log("Crataegutt test completed successfully.");
            break;
        } catch (error) {
            console.error('Error in runCrataeguttTest:', error);
            const is503Error = await checkFor503Error(page);
            if (is503Error) {
                console.log("TYPO3 503 error detected in Crataegutt test. Retrying...");
                retryCount++;
                await sleep(5000);
            } else {
    ...
    if (retryCount >= maxRetries) {
        throw new Error(`Failed to complete Crataegutt test after ${maxRetries} retries.`);
    }
}
