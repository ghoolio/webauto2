...
async function runHametumTest(page) {
    let retryCount = 0;
    const maxRetries = 3;

    while (retryCount < maxRetries) {
        try {
            console.log("Starting Hametum test...");
            await page.goto("https://www.schwabe-fachkreise.de/fortbildungen/online-trainings/produkt/hametum-haemorrhoiden/training/hametum-haemorrhoiden-erklaervideo/wissenstest/1/", {
            ...
            console.log("Hametum test completed successfully.");
            break;
        } catch (error) {
            console.error('Error in runHametumTest:', error);
            const is503Error = await checkFor503Error(page);
            if (is503Error) {
                console.log("TYPO3 503 error detected in Hametum test. Retrying...");
                retryCount++;
                await sleep(5000);
            } else {
    ...
    if (retryCount >= maxRetries) {
        throw new Error(`Failed to complete Hametum test after ${maxRetries} retries.`);
    }
}
