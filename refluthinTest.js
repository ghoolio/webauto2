...
async function runRefluthinTest(page) {
    let retryCount = 0;
    const maxRetries = 3;

    while (retryCount < maxRetries) {
        try {
            console.log("Starting Refluthin test...");
            await page.goto("https://www.schwabe-fachkreise.de/fortbildungen/online-trainings/produkt/refluthin/training/refluthin/wissenstest/1/", {
            ...
            console.log("Refluthin test completed successfully.");
            break;
        } catch (error) {
            console.error('Error in runRefluthinTest:', error);
            const is503Error = await checkFor503Error(page);
            if (is503Error) {
                console.log("TYPO3 503 error detected in Refluthin test. Retrying...");
                retryCount++;
                await sleep(5000);
            } else {
    ...
    if (retryCount >= maxRetries) {
        throw new Error(`Failed to complete Refluthin test after ${maxRetries} retries.`);
    }
}
