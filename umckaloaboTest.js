...
async function runUmckaloaboTest(page) {
    let retryCount = 0;
    const maxRetries = 3;

    while (retryCount < maxRetries) {
        try {
            console.log("Starting Umckaloabo test...");
            await page.goto("https://www.schwabe-fachkreise.de/fortbildungen/online-trainings/produkt/umckaloabo/training/umckaloabo-beratungstraining/wissenstest/1/", {
            ...
            console.log("Umckaloabo test completed successfully.");
            break;
        } catch (error) {
            console.error('Error in runUmckaloaboTest:', error);
            const is503Error = await checkFor503Error(page);
            if (is503Error) {
                console.log("TYPO3 503 error detected in Umckaloabo test. Retrying...");
                retryCount++;
                await sleep(5000);
            } else {
    ...
    if (retryCount >= maxRetries) {
        throw new Error(`Failed to complete Umckaloabo test after ${maxRetries} retries.`);
    }
}
