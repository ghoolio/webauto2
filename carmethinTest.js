...
async function runCarmenthinTest(page) {
    let retryCount = 0;
    const maxRetries = 3;

    while (retryCount < maxRetries) {
        try {
            console.log("Starting Carmenthin test...");
            await page.goto("https://www.schwabe-fachkreise.de/fortbildungen/online-trainings/produkt/carmenthin/training/carmenthin-wirkmechanismus/wissenstest/1/", {
            ...
            for (let i = 1; i <= 10; i++) {
                ...
                if (!currentUrl.includes('/frage/') && !currentUrl.includes('/keineinfoseite/')) {
                    ...
                    if (!questionElement) {
                        ...
                        await page.goto("https://www.schwabe-fachkreise.de/fortbildungen/online-trainings/produkt/carmenthin/training/carmenthin-basistraining/wissenstest/1/", {
                            waitUntil: "networkidle0",
                            timeout: 60000
                        });
                        await sleep(5000);
            ...
            // In carmethinTest.js, update the survey section:
            ...
            if (isSurveyPresent) {
                ...
                try {
                    ...
                    // Don't rethrow the error, allow the test to continue
                }
            } else {
                console.log("Survey not present after 3 attempts, skipping survey section");
            }

            console.log("Carmenthin test completed successfully.");
            break;
        } catch (error) {
            console.error('Error in runCarmenthinTest:', error);
            const is503Error = await checkFor503Error(page);
            if (is503Error) {
                console.log("TYPO3 503 error detected in Carmenthin test. Retrying...");
                retryCount++;
                await sleep(5000);
            } else {
    ...
    if (retryCount >= maxRetries) {
        throw new Error(`Failed to complete Carmenthin test after ${maxRetries} retries.`);
    }
}
