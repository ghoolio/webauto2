...
async function runProstaguttTest(page) {
    let retryCount = 0;
    const maxRetries = 3;

    while (retryCount < maxRetries) {
        try {
            console.log("Starting Prostagutt test...");
            await page.goto("https://www.schwabe-fachkreise.de/fortbildungen/online-trainings/produkt/prostagutt-duo/training/prostagutt-duo-training/wissenstest/1/", {
            ...
            console.log("Prostagutt test completed successfully.");
            break;
        } catch (error) {
            console.error('Error in runProstaguttTest:', error);
            const is503Error = await checkFor503Error(page);
            if (is503Error) {
                console.log("TYPO3 503 error detected in Prostagutt test. Retrying...");
                retryCount++;
                await sleep(5000);
            } else {
    ...
    if (retryCount >= maxRetries) {
        throw new Error(`Failed to complete Prostagutt test after ${maxRetries} retries.`);
    }
}
...
// #question > div:nth-child(2) > div > div > a
